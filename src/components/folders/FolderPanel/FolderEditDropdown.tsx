...
interface FolderEditDropdownProps {
  folder: Folder;
  folders: Folder[];
  onClose: () => void;
  onUpdateFolder: (
...
export default function FolderEditDropdown({
  folder,
  folders,
  onClose,
  onUpdateFolder,
  ...
  onCreateSubfolder,
  onOpenDeleteModal,
}: FolderEditDropdownProps) {
  const [name, setName] = useState(folder.name);
  const [selectedColor, setSelectedColor] = useState(folder.color ?? '#6b7280');
  ...
  const availableParentsWithPaths = useMemo(() => {
    // Use getFolderOptionsWithPaths which already filters out invalid parents
  ...
  }, [folder.id, selectedParentId, folder.parentId, folders]);
  ...
  const handleSave = async () => {
    if (!name.trim() || isSaving || !hasChanges) return;
  ...
  return (
    <div
      className="min-w-[320px] space-y-4 p-4"
    ...
    >
      <div className="border-border flex items-center gap-2 border-b pb-2">
        ...
        />
        <span className="font-medium">{name || 'Folder'}</span>
      </div>
        ...
        <div className="grid grid-cols-6 gap-2">
          {FOLDER_COLORS.map((color) => (
            <button
              ...
              disabled={isSaving}
              className={`relative h-8 w-8 rounded-lg border-2 transition-all hover:scale-105 ${
                selectedColor === color
