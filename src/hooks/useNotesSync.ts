...
export function useNotesSync({
  ...
  const createUpdatedNote = useCallback((baseNote: Note, syncedNote: Note) => {
    const updatedNote = { ...syncedNote };
    updatedNote.attachments = baseNote.attachments;
  ...
  // Note sync handlers
  const onNoteSync = useCallback((message: NoteSyncMessage) => {
    // Update the note in local state with the synced changes
    setNotes((prevNotes) =>
      prevNotes.map((note) => {
        if (note.id === message.noteId) {
          return createUpdatedNote(note, message.updatedNote);
        }
        return note;
      })
    );
    ...
    setSelectedNote((prevSelected) => {
      if (prevSelected?.id === message.noteId) {
        const updatedNote = createUpdatedNote(prevSelected, message.updatedNote);
  ...
  const onNoteCreatedSync = useCallback((message: NoteCreatedSyncMessage) => {
  ...
  const onNoteDeletedSync = useCallback((message: NoteDeletedSyncMessage) => {
  ...
  const onFolderCreatedSync = useCallback((message: FolderCreatedSyncMessage) => {
  ...
  const onFolderUpdatedSync = useCallback((message: FolderUpdatedSyncMessage) => {
  ...
  const onFolderDeletedSync = useCallback((message: FolderDeletedSyncMessage) => {
    setFolders((prevFolders) =>
