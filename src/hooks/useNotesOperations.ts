...
export function useNotesOperations({
  ...
  const createNote = useCallback(async (folderId?: string, templateContent?: { title: string; content: string }) => {
    try {
      if (!encryptionReady) {
        throw new Error(
  ...
  }, [encryptionReady, selectedFolder?.id, folders, convertApiNote, setNotes, setSelectedNote, webSocket, setError]);
  ...
  }, [encryptionReady, selectedNote?.id, selectedNote?.attachments, folders, loadData, convertApiNote, setNotes, setSelectedNote, webSocket, setError]);
  ...
  }, [convertApiNote, selectedNote, setNotes, setSelectedNote, webSocket, setError]);
  ...
  const hideNote = useCallback(async (noteId: string) => {
    try {
      ...
      // Send WebSocket update for real-time sync
      if (webSocket.isAuthenticated) {
        webSocket.sendNoteUpdate(noteId, { hidden: hiddenNote.hidden });
      }
    } catch (error) {
  ...
  const unhideNote = useCallback(async (noteId: string) => {
    try {
      ...
      // Send WebSocket update for real-time sync
      if (webSocket.isAuthenticated) {
        webSocket.sendNoteUpdate(noteId, { hidden: unhiddenNote.hidden });
      }
    } catch (error) {
  ...
  // Folder operations
  const createFolder = useCallback(async (name: string, color: string = '#3b82f6', parentId?: string) => {
    try {
  ...
  }, [folders, selectedFolder, getDescendantIds, setFolders, setSelectedNote, webSocket, setError]);
